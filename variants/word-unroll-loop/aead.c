...
int crypto_aead_encrypt(unsigned char* c, unsigned long long* clen,
  ...
  *clen = mlen + CRYPTO_ABYTES;
  ...
  // =====optimization=====
  // unroll the loop
  const uint64_t K0 =  LOADLOWER4BYTES(k);// LOADBYTES(k + 0, 4) >> 32;
  const uint64_t K1 = LOAD8BYTES(k+4);// LOADBYTES(k + 4, 8);
  const uint64_t K2 = LOAD8BYTES(k+12);// LOADBYTES(k + 12, 8);
  const uint64_t N0 = LOAD8BYTES(npub);// LOADBYTES(npub, 8);
  const uint64_t N1 =  LOAD8BYTES(npub+8);//LOADBYTES(npub + 8, 8);
  ...
  if (adlen) {
    ...
    while (adlen >= ASCON_128_RATE) {
        //======optimization=====
      s.x[0] ^= LOAD8BYTES(ad);// LOADBYTES(ad, 8);
  ...
  while (mlen >= ASCON_128_RATE) {
      // =====optimization=====
    s.x[0] ^=  LOAD8BYTES(m);// LOADBYTES(m, 8);
                             //
...
int crypto_aead_decrypt(unsigned char* m, unsigned long long* mlen,
  ...
  */
  ...
  while (clen >= ASCON_128_RATE) {
    // =====optimization=====
    uint64_t c0 = LOAD8BYTES(c);//LOADBYTES(c, 8);
    STORE8BYTES(m,s.x[0]^c0);
