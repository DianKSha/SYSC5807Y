...
int crypto_aead_encrypt(unsigned char* c, unsigned long long* clen,
  ...
  *clen = mlen + CRYPTO_ABYTES;
  ...
  //Reduce code complexity: Make the code easier to read and maintain through clear macro definitions.

  //Improve performance: Reduce unnecessary bit operations, although it depends on specific hardware and compiler optimization, it may usually be more efficient to load directly.

  //Data alignment: Directly use pointer type conversion to assume that the data has been aligned according to the hardware requirements, so that it is possible to use the optimized loading instructions of the hardware.
#define LOAD64(ptr) (*((const uint64_t*)(ptr))) // Assume that the data has been correctly aligned.
#define LOAD32TO64_HIGH(ptr) (*((const uint32_t*)(ptr)) << 32) // Load 32-bit data to the high bit of 64-bit variables

const uint64_t K0 = LOAD32TO64_HIGH(k + 0);
const uint64_t K1 = LOAD64(k + 4);
const uint64_t K2 = LOAD64(k + 12);
const uint64_t N0 = LOAD64(npub);
const uint64_t N1 = LOAD64(npub + 8);
