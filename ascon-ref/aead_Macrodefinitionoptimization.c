/// Macro definition LOADBYTES and STOREBYTES: Use macros to directly operate memory, reduce function call overhead, and use bit operations to improve the efficiency of data processing.
...
#define LOADBYTES(src, n) (*((uint64_t*)(src)) & ((1ULL << ((n) * 8)) - 1))
#define STOREBYTES(dest, src, n) \
    do { \
        uint64_t mask = (1ULL << ((n) * 8)) - 1; \
        *((uint64_t*)(dest)) = (*((uint64_t*)(dest)) & ~mask) | ((src) & mask); \
    } while(0)
...
// Use macro optimization to load keys and nonce
...
// Simplified data processing logic
  size_t blocks = adlen / ASCON_128_RATE;
  for (size_t i = 0; i < blocks; i++) {
      s.x[0] ^= LOADBYTES(ad + i * ASCON_128_RATE, 8);
      P6(&s);
  }
  // Handle the last incomplete block.
  if (adlen % ASCON_128_RATE) {
      s.x[0] ^= LOADBYTES(ad + blocks * ASCON_128_RATE, adlen % ASCON_128_RATE);
      s.x[0] ^= PAD(adlen % ASCON_128_RATE);
      P6(&s);
  }
  /* domain separation */
  s.x[4] ^= 1;
